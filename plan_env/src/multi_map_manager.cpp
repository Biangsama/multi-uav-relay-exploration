...
bool MultiMapManager::hasRemoteChunkUpdate() const {
  return remote_chunk_update_;
}

bool MultiMapManager::consumeRemoteChunkUpdate() {
  const bool had_update = remote_chunk_update_;
  remote_chunk_update_ = false;
  remote_chunk_insert_count_ = 0;
  return had_update;
}

void MultiMapManager::chunkTimerCallback(const ros::TimerEvent& e) {
  bool inserted_remote_chunks = false;
  int inserted_chunk_count = 0;
  ...
  if (inserted_remote_chunks) {
    remote_chunk_update_ = true;
    remote_chunk_insert_count_ += inserted_chunk_count;
    ROS_INFO_STREAM_THROTTLE(1.0, "MultiMapManager received remote chunks: "
                                      << inserted_chunk_count
                                      << ", buffered frontier refresh pending.");
  }
