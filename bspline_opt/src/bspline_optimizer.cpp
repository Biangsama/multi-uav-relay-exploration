...
bool BsplineOptimizer::optimize(Eigen::MatrixXd& points, double& dt, const int& cost_function,
...
bool BsplineOptimizer::optimize() {
  ...
  try {
    ...
    ROS_WARN_STREAM("NLopt optimize failed: " << e.what() << ", dim=" << dim_
                    << ", point_num=" << point_num_ << ", variable_num=" << variable_num_
                    << ", knot_span=" << knot_span_);
  }

  if (best_variable_.size() != static_cast<size_t>(variable_num_)) {
    ROS_ERROR_STREAM("Invalid best_variable size after optimization: got=" << best_variable_.size()
                     << ", expect=" << variable_num_ << ". Keeping original control points.");
    return false;
  }

  for (int i = 0; i < point_num_; ++i) {
    for (int j = 0; j < dim_; ++j) {
      const double value = best_variable_[dim_ * i + j];
      if (!std::isfinite(value)) {
        ROS_ERROR_STREAM("Non-finite optimized control point at (" << i << ", " << j
                         << ") during B-spline optimization. Keeping original control points.");
        return false;
      }
      control_points_(i, j) = value;
    }
  }
  if (optimize_time_) {
    const double new_knot_span = best_variable_[variable_num_ - 1];
    if (!std::isfinite(new_knot_span) || new_knot_span <= 0.0) {
      ROS_ERROR_STREAM("Invalid optimized knot span=" << new_knot_span
                       << ". Keeping original knot span.");
      return false;
    }
    knot_span_ = new_knot_span;
  }
