...
public:
    ...
    virtual void send_double_array(const double* values, int size) = 0;
...
{
public:
    explicit UDSSocket(boost::asio::io_context& io_context)
        : socket_(io_context)
    {
    }
    ...
    void accept(const std::string& host, unsigned short) override
    {
        ...
        boost::asio::local::stream_protocol::acceptor acceptor(
            ...
            boost::asio::local::stream_protocol::endpoint(host));
    ...
    void connect(const std::string& host) override
    {
        ...
        for (int i = 0; i < 20; ++i)
        {
            ...
            {
                std::cerr << "Error connecting to socket on try " << i << " : " << e.what()
                          << std::endl;
    ...
    {
        uint32_t send_length = htonl(static_cast<uint32_t>(message.size()));
    ...
    void send_double_array(const double* values, int size) override
    {
...
public:
    ...
    std::string receive_one_message() override
    {
        ...
        std::string data(receive_length, '\0');
        boost::asio::read(socket_, boost::asio::buffer(&data[0], receive_length));
        return data;
    }
