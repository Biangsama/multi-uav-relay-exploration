...
UniformGrid::UniformGrid(
  ...
  nh.param("partitioning/fine_grid_requires_frontier", fine_grid_requires_frontier_, true);
...
void UniformGrid::updateGridData(const int& drone_id, vector<int>& grid_ids, vector<int>& parti_ids,
  ...
  for (int i = 0; i < grid_data_.size(); ++i) {
    ...
    for (const auto& pair : grid.frontier_cell_nums_) {
      frontier_cells += pair.second;
    }
    ...
    if (!grid.is_prev_relevant_ && grid.is_cur_relevant_ && level_ > 1 &&
      ...
      ROS_WARN("Grid %d is rediscovered (frontiers=%d, frontier_cells=%d, unknown=%d)", i,
          static_cast<int>(grid.contained_frontier_ids_.size()), frontier_cells, grid.unknown_num_);
    }
  }
  ...
  if (!initialized_) {
    ...
    // Keep rediscovered fine grids globally active, but do not silently reassign them
    // to the local drone here. Let the shared unallocated-pool logic decide ownership so
    // revisits do not pull an explorer back into already-covered regions.
...
void UniformGrid::inputFrontiers(const vector<Eigen::Vector3d>& avgs, const vector<int>& cell_nums) {
  for (auto& grid : grid_data_) {
  ...
  for (int i = 0; i < avgs.size(); ++i) {
    ...
    const int frontier_cells = (i < cell_nums.size()) ? std::max(0, cell_nums[i]) : 0;
    grid.frontier_cell_nums_[i] = frontier_cells;
    grid.frontier_num_ = static_cast<int>(grid.contained_frontier_ids_.size());
  }
}
...
bool UniformGrid::isRelevant(const GridInfo& grid) {
  ...
  const bool frontier_supported = frontier_cells >= min_frontier_;

  if (level_ > 1 && fine_grid_requires_frontier_) {
    return frontier_supported;
  }

  return grid.unknown_num_ >= min_unknown_ || frontier_supported;
}
