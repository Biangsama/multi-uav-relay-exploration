...
void HGrid::inputFrontiers(const vector<Eigen::Vector3d>& avgs, const vector<int>& cell_nums) {
  // Input frontier to both levels
  grid1_->inputFrontiers(avgs, cell_nums);
  grid2_->inputFrontiers(avgs, cell_nums);
}
...
bool HGrid::isFineGrid(const int& grid_id) const {
  return grid_id >= grid1_->grid_data_.size();
}
