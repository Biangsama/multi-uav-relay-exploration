...
void FastPlannerManager::planYawExplore(const Eigen::Vector3d& start_yaw, const double& end_yaw,
  ...
  if (!std::isfinite(local_data_.duration_) || local_data_.duration_ <= 1e-3 || !std::isfinite(dt_yaw) ||
      dt_yaw <= 1e-3) {
    ROS_WARN_STREAM("Skip yaw optimization because of invalid duration in planYawExplore: duration="
                    << local_data_.duration_ << ", dt_yaw=" << dt_yaw);
    dt_yaw = 0.1;
  }
  ...
  bspline_optimizers_[1]->setBoundaryStates(start, end);
  bspline_optimizers_[1]->setWaypoints(waypts, waypt_idx);
  const bool yaw_opt_success = bspline_optimizers_[1]->optimize(yaw, dt_yaw, cost_func, 1, 1);
  if (!yaw_opt_success) {
    ROS_WARN_STREAM("Yaw optimization fallback in planYawExplore: duration=" << local_data_.duration_
                    << ", dt_yaw=" << dt_yaw << ", start_yaw=" << start_yaw3d.transpose()
                    << ", end_yaw=" << end_yaw3d.transpose() << ", waypoints=" << waypts.size()
                    << ". Reusing the pre-optimization yaw control points.");
  }
