...
private:
  ...
  ros::Publisher task_metrics_pub_;
  ...
  void publishTaskMetrics();
  bool tryPlanGoalCandidate(const Vector3d& start_pos, const Vector3d& candidate,
      bool optimistic, Vector3d& safe_goal, vector<Vector3d>& safe_path);
  bool findSafeReachableGoal(const Vector3d& start_pos, const Vector3d& raw_goal,
      bool optimistic, Vector3d& safe_goal, vector<Vector3d>& safe_path);
